...
static std::string FindConfigFile(const std::string& plyFilename, const std::string& configFilename)
{
    ...
    {
        ECS_LOG_ERROR("PLY file does not exist or is not a file: \"%s\"", plyFilename.c_str());
        return "";
    }
...
static std::shared_ptr<PointCloud> LoadPointCloud(const std::string& plyFilename)
{
    ...
    {
        ECS_LOG_ERROR("Error loading PointCloud!\n");
        return nullptr;
    }
    return pointCloud;
}
...
bool App::Init()
{
    ...
    {
        ECS_LOG_ERROR("DebugRenderer Init failed\n");
        return false;
    }
    ...
    {
        ECS_LOG_ERROR("TextRenderer Init failed\n");
        return false;
    }
    ...
    if (opt.vrMode)
    {
        ...
        if (!xrBuddy->Init())
        {
            ECS_LOG_ERROR("OpenXR Init failed\n");
            return false;
        }
    }
    ...
    {
        ECS_LOG_ERROR("Error initalizing MagicCarpet\n");
        return false;
    }
    ...
    if (!pointCloudFilename.empty())
    {
        ...
        {
            ECS_LOG_ERROR("Error initializing point renderer!\n");
            return false;
        }
    }
    else
    ...
    if (opt.vrMode)
    {
        ...
        {
            ECS_LOG_ERROR("Error loading desktop shader!\n");
            return 1;
        }
    ...
    inputBuddy->OnKey(SDLK_RETURN, [this, vrConfigFilename](bool down, uint16_t mod)
    {
        if (down && opt.vrMode)
        {
            ...
            {
                ECS_LOG_ERROR("Writing \"%s\" failed\n", vrConfigFilename.c_str());
            }
        }
    });
...
bool App::Process(float dt)
{
    if (opt.vrMode)
    {
        ...
        {
            ECS_LOG_ERROR("xrBuddy PollEvents failed\n");
            return false;
        }

        if (!xrBuddy->SyncInput())
        {
            ECS_LOG_ERROR("xrBuddy SyncInput failed\n");
            return false;
        }
...
bool App::Render(float dt, const glm::ivec2& windowSize)
{
    ...
    if (opt.vrMode)
    {
        if (xrBuddy->SessionReady())
        {
            ...
            {
                ECS_LOG_ERROR("xrBuddy RenderFrame failed\n");
                return false;
            }
        }
        else
