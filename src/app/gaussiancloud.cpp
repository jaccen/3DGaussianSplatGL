...
bool GaussianCloud::ImportPly(const std::string& plyFilename)
{
    ...
    if (!plyFile.is_open())
    {
        ECS_LOG_ERROR("failed to open %s\n", plyFilename.c_str());
        return false;
    }
    ...
    {
        ECS_LOG_ERROR("Error parsing ply file \"%s\"\n", plyFilename.c_str());
        return false;
    }
    ...
    {
        ECS_LOG_ERROR("Error parsing ply file \"%s\", missing position property\n", plyFilename.c_str());
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            ECS_LOG_ERROR("Error parsing ply file \"%s\", missing f_dc property\n", plyFilename.c_str());
        }
    }
    ...
    for (int i = 0; i < 45; i++)
    {
        ...
        {
            ECS_LOG_ERROR("Error parsing ply file \"%s\", missing f_rest property\n", plyFilename.c_str());
        }
    }

    if (!ply.GetProperty("opacity", props.opacity))
    {
        ECS_LOG_ERROR("Error parsing ply file \"%s\", missing opacity property\n", plyFilename.c_str());
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            ECS_LOG_ERROR("Error parsing ply file \"%s\", missing scale property\n", plyFilename.c_str());
        }
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            ECS_LOG_ERROR("Error parsing ply file \"%s\", missing rot property\n", plyFilename.c_str());
        }
    }
