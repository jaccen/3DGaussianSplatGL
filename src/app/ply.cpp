...
bool Ply::CheckLine(std::ifstream& plyFile, const std::string& validLine)
{
    std::string line;
    return std::getline(plyFile, line) && line == validLine;
}

bool Ply::GetNextPlyLine(std::ifstream& plyFile, std::string& lineOut)
{
...
bool Ply::Parse(std::ifstream& plyFile)
{
    ...
    {
        ECS_LOG_ERROR("Unexpected error reading next line\n");
        return false;
    }
    if (token1 != "ply")
    {
        ECS_LOG_ERROR("Invalid ply file\n");
        return false;
    }
    ...
    {
        ECS_LOG_ERROR("Invalid ply file, expected format\n");
        return false;
    }
    if (token1 != "format binary_little_endian 1.0")
    {
        ECS_LOG_ERROR("Unsupported ply file, only binary_little_endian supported\n");
        return false;
    }
    ...
    {
        ECS_LOG_ERROR("Invalid ply file, expected \"element vertex {number}\"\n");
        return false;
    }
    ...
    while (true)
    {
        if (!GetNextPlyLine(plyFile, line))
        {
            ECS_LOG_ERROR("unexpected error reading line\n");
            return false;
        }
        ...
        if (token1 != "property")
        {
            ECS_LOG_ERROR("Invalid header, expected property\n");
            return false;
        }
        ...
        {
            ECS_LOG_ERROR("Unsupported type \"%s\" for property \"%s\"\n", token2.c_str(), token3.c_str());
            return false;
        }
    }
