                         ...
                         bool useFullSHIn)
{
    ...
    splatProg = std::make_shared<jaccen::gfx::Shader>();
    ...
    {
        ECS_LOG_ERROR("Error loading splat shaders!\n");
        return false;
    }
    ...
    {
        ECS_LOG_ERROR("Error loading point pre-sort compute shader!\n");
        return false;
    }
...
void SplatRenderer::BuildVertexArrayObject(std::shared_ptr<GaussianCloud> gaussianCloud)
{
    ...
    auto positionBuffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, posVec);

    auto r_sh0Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, r_sh0Vec);
    auto g_sh0Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, g_sh0Vec);
    auto b_sh0Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, b_sh0Vec);

    auto r_sh1Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, r_sh1Vec);
    auto r_sh2Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, r_sh2Vec);
    auto r_sh3Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, r_sh3Vec);
    auto g_sh1Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, g_sh1Vec);
    auto g_sh2Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, g_sh2Vec);
    auto g_sh3Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, g_sh3Vec);
    auto b_sh1Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, b_sh1Vec);
    auto b_sh2Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, b_sh2Vec);
    auto b_sh3Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, b_sh3Vec);

    auto cov3_col0Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, cov3_col0Vec);
    auto cov3_col1Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, cov3_col1Vec);
    auto cov3_col2Buffer = std::make_shared<jaccen::BufferObject>(GL_ARRAY_BUFFER, cov3_col2Vec);
    ...
    auto indexBuffer = std::make_shared<jaccen::BufferObject>(GL_ELEMENT_ARRAY_BUFFER, indexVec, GL_DYNAMIC_STORAGE_BIT);
